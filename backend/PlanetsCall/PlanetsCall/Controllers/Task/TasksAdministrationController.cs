...
public class TasksAdministrationController(ITasksRepository tasksRepository, IOrganisationsRepository organisationsRepository, IServiceScopeFactory serviceScopeFactory)
    : ControllerBase
{
      [HttpPost]
      [Route("template-task/")]
      ...
      public IActionResult GetTasks() // get all tasks created by admins (type 1 and 2)
      {
          ...
          return Ok(tasks.Select(t => new FullTaskDto(t)));
      }
      ...
      public IActionResult GetOrganizationTasks(string organizationName) // get all tasks created in organizations (type 3)
      {
          ...
          return Ok(tasksRepository.GetOrganizationTasks(organisation).Select(t => new FullTaskDto(t)));
      }
      ...
      public async Task<IActionResult> AddOrganizationTask(string organizationName, [FromBody] TaskInfo task) // create task (as an organization member with rights)
      {
          ...
          try
          {
              ...
              if (requestUser != null)
              {
                  ...
                  Tasks newTask = tasksRepository.CreateTask(task, requestUser, organisation); // create task
